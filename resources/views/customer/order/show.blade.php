...
@section('content')
    <div class="pb-20">
        ...
        <div class="bg-white p-4 border-b border-gray-100 flex items-center">
            <a href="{{ route('customer.orders.index') }}" class="mr-3">
                <svg class="w-6 h-6 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                </svg>
            </a>
            <div>
                <h1 class="text-lg font-bold text-brew-dark">{{ $order->order_number }}</h1>
        ...
        @endphp
        <div class="bg-gradient-to-r {{ $statusColors[$order->status] ?? 'from-gray-400 to-gray-500' }} p-6 text-white text-center">
            <div class="text-5xl mb-3">{{ $statusInfo['icon'] }}</div>
            <h2 class="text-2xl font-bold">{{ $statusInfo['label'] }}</h2>
            <p class="text-white/80 mt-1">{{ $statusInfo['description'] }}</p>
        </div>
                ...
                @foreach($steps as $index => $step)
                    <div class="flex flex-col items-center">
                        <div class="w-10 h-10 rounded-full flex items-center justify-center text-lg
                            {{ $index <= $currentIndex ? 'bg-brew-gold' : 'bg-gray-200' }}">
                            {{ $step['icon'] }}
                        </div>
                        <span class="text-xs mt-1 {{ $index <= $currentIndex ? 'text-brew-dark font-semibold' : 'text-gray-400' }}">
                            {{ $step['label'] }}
                        </span>
                    </div>
                    @if($index < count($steps) - 1)
                        <div class="flex-1 h-1 mx-2 rounded {{ $index < $currentIndex ? 'bg-brew-gold' : 'bg-gray-200' }}"></div>
                    @endif
                @endforeach
            </div>
        </div>
        ...
        <div class="p-4 space-y-4">
            ...
            <div class="bg-white rounded-xl shadow-sm p-4">
                <h3 class="font-semibold text-brew-dark mb-3">👤 Informasi</h3>
            ...
            {{-- Order Items --}}
            <div class="bg-white rounded-xl shadow-sm p-4">
                <h3 class="font-semibold text-brew-dark mb-3">🛍️ Pesanan</h3>
                <div class="space-y-3">
            ...
            {{-- Payment Summary --}}
            <div class="bg-white rounded-xl shadow-sm p-4">
                <h3 class="font-semibold text-brew-dark mb-3">💳 Pembayaran</h3>
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
        ...
        @if($order->status === 'ready')
            <div class="p-4">
                <div class="bg-green-50 border border-green-200 rounded-xl p-4 text-center">
                    ...
                    <p class="font-semibold text-green-800">Pesananmu sudah siap!</p>
                    <p class="text-green-700 text-sm">Silakan ambil di counter dengan menyebutkan nomor pesanan</p>
                </div>
            </div>
        @endif
    </div>
